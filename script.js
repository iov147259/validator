'use strict';

// Код валидации формы
(function () {
    function validateNumber(value, min, max) {
        let val = parseInt(value);
        return (val < min || val > max || isNaN(val)) ? false : true;
    };

    function validateRegEx(value, pattern, flag) {
        var reg = RegExp(pattern, flag);
        return reg.test(value)
    }

    function validateValue(value, dataset) {
        switch (dataset.validator) {
            case 'number':
                return validateNumber(value, dataset.validatorMin, dataset.validatorMax);
            case 'letters':
                return validateRegEx(value, '^[a-zа-яё]+$', "i");
            case 'regexp':
                return validateRegEx(value, dataset.validatorPattern);
            default:
                return true;
        }
    }

    function checkReqired(input) {
        var value = input.value;
        if (input.dataset.hasOwnProperty('required') && !value) {
            return false;
        }
        var validator = input.dataset.validator;
        return (validator && value) ? validateValue(value, input.dataset) : true;
    };

    window.validateForm = function (options) {
        var form = document.getElementById(options.formId);
        var inputs = Array.from(document.querySelectorAll('#' + options.formId + " input"));
        form.addEventListener("focus", function (event) {
            var target = event.target;
            if (target.tagName === "INPUT") {
                target.classList.remove(options.inputErrorClass);
            }

        }, true);
        form.addEventListener('blur', function (event) {
            var target = event.target;
            if (target.tagName === "INPUT" && !checkReqired(target)) {
                form.classList.add(options.inputErrorClass)
            }
        }, true);

          form.addEventListener("submit",function (event) {
              event.preventDefault();
              var target= event.target;
              form.classList.remove(options.formValidClass);
              form.classList.remove(options.formInvalidClass);
              var isInfalid=false;
              inputs.forEach( function (input) {
                 if(!checkReqired(input)) {
                     input.classList.add(options.inputErrorClass);
                     isInfalid=true;
                 };
              });
              (isInfalid)?form.classList.add(options.formInvalidClass):form.classList.add(options.formValidClass);

          });
    };

}());
